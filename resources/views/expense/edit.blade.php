...
<div class="card card-flush h-lg-100" id="kt_contacts_main">
	...
	<form method="POST" action="{{ route('expenses.update',$expense->id) }}">
        ...
        @method('put')
			...
			<div class="row">
				<div class="col-sm-4 mt-6">
					...
					<input type="number"
						...
						name="amount" id="total_price" value="{{$expense->amount}}" placeholder="المبلغ للدفع">
				</div>
				...
				<div class="col-sm-4 mt-4">
					...
					name="currency" data-control="select2" data-placeholder="Select an option" data-allow-clear="true">
						<option value="شيكل" {{ $expense->currency == "شيكل" ? 'selected' : '' }} >شيكل</option>
						<option value="دولار" {{ $expense->currency == "دولار" ? 'selected' : '' }}>دولار</option>
						<option value="دينار"  {{ $expense->currency == "دينار" ? 'selected' : '' }} >دينار</option>
					</select>
				...
				<div class="col-4 mt-4">
					...
					<input type="date" name="pay_date"
						...
						placeholder="التاريخ " value="{{$expense->pay_date }}">
				</div>
			</div>
			<div>
			...
			placeholder="ملاحظات">{{$expense->notes}}</textarea>
			</div>
			<div class="d-flex">
